...
class UnitActivityReport
  ...
  def upload_uar
    file.each_with_index(row_params) do |row, index|
      ...
      if valid_row?(index, row)
        ...
        potential_new_activity = create_activity(product_in_row, row)
        potential_new_activity.save if potential_new_activity.valid?
  ...
  def create_activity(product_in_row, row)
    ...
    product_in_row.activities.build(sold: row[:sold], date: create_datetime)
  end
